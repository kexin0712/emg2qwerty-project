...
class MagnitudeWarping:
    """Applies smooth random amplitude scaling over time.

    This transform perturbs signal magnitude by multiplying with a smooth
    random curve over the temporal axis. By default, each feature channel gets
    an independent curve.

    Args:
        sigma (float): Std-dev of multiplicative variation around 1.0.
            (default: 0.2)
        knot (int): Number of internal control points for the smooth curve.
            (default: 4)
        independent_channels (bool): If True, apply independent curves per
            channel/feature. Otherwise use one shared curve. (default: True)
        time_dim (int): Time dimension index. (default: 0)
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        time_dim = self.time_dim if self.time_dim >= 0 else tensor.ndim + self.time_dim
        assert 0 <= time_dim < tensor.ndim

        x = tensor.movedim(time_dim, 0)
        T = x.shape[0]
        flat = x.reshape(T, -1)
        num_features = flat.shape[1]
        if T <= 1 or num_features == 0:
            return tensor

        num_curves = num_features if self.independent_channels else 1
        n_ctrl = self.knot + 2
        control = torch.randn(
            num_curves,
            n_ctrl,
        ...
        # Keep scaling positive and numerically stable.
        control = control.clamp_min(1e-3)

        curves = F.interpolate(
            control.unsqueeze(1), size=T, mode="linear", align_corners=True
        ).squeeze(1)
        if not self.independent_channels:
            curves = curves.expand(num_features, T)

        warped = flat * curves.transpose(0, 1)
        return warped.reshape_as(x).movedim(0, time_dim)
...
@dataclass
class RandomTemporalAttenuation:
    def __init__(
        self,
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        max_width = min(self.max_width, T)
        min_width = min(self.min_width, max_width)

        if min_width < 1 or max_width < 1:
            return x

        width = int(torch.randint(low=min_width, high=max_width + 1, size=(1,)).item())
        if width >= T:
            start = 0
            end = T
        else:
            start = int(torch.randint(low=0, high=T - width + 1, size=(1,)).item())
            end = start + width
