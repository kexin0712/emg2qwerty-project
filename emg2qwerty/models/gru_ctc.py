...
class GRUCTCModule(CTCModelBase, pl.LightningModule):
    ...
    def __init__(
        ...
        gru_dropout = gru_dropout if gru_num_layers > 1 else 0.0
        ...
        self.encoder_norm = (
            nn.LayerNorm(2 * gru_hidden_size) if use_layer_norm else nn.Identity()
        )
        self.output_dropout = nn.Dropout(output_dropout)
        self.classifier = nn.Sequential(
            nn.Linear(2 * gru_hidden_size, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
